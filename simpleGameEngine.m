...
classdef simpleGameEngine < handle
    ...
    methods
        ...
        function [row,col,button] = getMouseInput(obj)
            ...
            %% Changed by Noah Klein
            try
                [X,Y,button] = ginput(1);
            catch
                warning('Irregular Key Pressed, Assigning values as if no key pressed');
                %Use deal to assign all values in array to 0
                [X,Y,button] = deal(0);
            end

            %Prevent the function from returning empty value ruining game logic
            if(isempty(button))
                button = 0;
            end
            %%
